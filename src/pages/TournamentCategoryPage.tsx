...
const TournamentCategoryPage = () => {
  ...
  const { tournaments: fetchedTournaments, loading, error } = useTournaments();

  // Filter tournaments based on the category URL parameter
  const tournaments = fetchedTournaments?.filter((tournament) => {
    if (category === "open") return tournament.status === "open";
    if (category === "in-progress") return tournament.status === "in-progress";
    if (category === "completed") return tournament.status === "completed";
    return true;
  });
